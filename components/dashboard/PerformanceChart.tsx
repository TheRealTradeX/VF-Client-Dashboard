...
type PerformancePoint = { date: string; balance: number; equity: number };
...
export function PerformanceChart({ data, rangeLabel }: { data?: PerformancePoint[]; rangeLabel?: string }) {
    ...
    <div className="bg-zinc-950 border border-zinc-900 rounded-xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-500">{rangeLabel ?? 'Last 30 days'}</p>
        </div>
      ...
      {chartData.length === 0 ? (
        <div className="text-sm text-zinc-500">No performance data yet.</div>
      ) : (
